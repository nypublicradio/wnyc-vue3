<script setup>
import { isolateSlug } from "~/utilities/helpers"
...
const {
  ...
  status,
  error,
} = useAsyncData(`recent-episodes-${props.show?.slug}`, async () => {
  const redirects = Array.isArray(cachedRedirects.value)
    ? cachedRedirects.value
    : []
