...
const props = defineProps({
  ...
  source: {
    type: String,
    default: 'wnyc_newsletter_landing_page',
  },
  altDesign: {
    type: Boolean,
    default: false,
...
<template>
  <div v-if="showComponent" class="newsletter-home grid" :class="[{ small }]">
    <Button v-if="showX" aria-label="Dismiss newsletter signup form"
      class="closer-x p-button-rounded p-button-sm p-button-text" icon="pi pi-times" @click="hideComp" />
    <div class="col-12 lg:col-3 pr-3 pb-2" :class="showX ? 'pr-6' : 'pr-3'">
      <div class="h4">
        {{ titleText }}
      </div>
    </div>
    <div class="col-12 lg:col-7 xl:col-5 col-offset-0 xl:col-offset-1">
      ...
      <email-collector-form :class="showBlurb ? 'mt-5' : 'mt-2'" :show-no-thanks="showNoThanks"
        :submit-button-text="submitButtonText" :submit-button-icon="submitButtonIcon" :is-submitting="isSubmitting"
        :submission-status="submissionStatus" :alt-design="altDesign" :outlined="outlined" @no-thanks-click="hideComp"
        @submit="submitForm">
